...
var require_lib = __commonJS({
  "node_modules/obsidian-dataview/lib/index.js"(exports) {
    ...
    function chainOpt(base, ...funcs) {
      return parsimmon_umd_min.exports.custom((success2, failure2) => {
        return (input, i) => {
...
function enabledCommunityPlugins() {
  var _a;
  const list = Array.from(((_a = window.app.plugins) == null ? void 0 : _a.enabledPlugins) || []);
  return list.sort();
}
...
function getDailyNotePathIfPluginIsAvailable() {
  ...
  return dailyNote ? success(dailyNote.path) : failure(404, STRINGS.note_not_found);
}
function getFileMap() {
  const { vault } = window.app;
  const { fileMap } = vault;
...
async function renameFilepath(filepath, newFilepath) {
  ...
  try {
    ...
    return failure(409, msg.contains("no such file or directory") ? "No such file or folder" : msg);
  }
...
var zodExistingFilePath = z.preprocess(lookupAbstractFileForPath, z.instanceof(import_obsidian2.TFile, { message: "File doesn't exist" }));
var zodExistingFolderPath = z.preprocess(lookupAbstractFolderForPath, z.instanceof(import_obsidian2.TFolder, { message: "Folder doesn't exist" }));
...
var createParams = z.discriminatedUnion("apply", [
...
async function handleOpenCurrent(incomingParams) {
  const res = getDailyNotePathIfPluginIsAvailable();
  return res.isSuccess ? success({ message: STRINGS.open.note_opened }, res.result) : res;
}
async function handleOpenMostRecent(incomingParams) {
  const res = await getMostRecentDailyNote();
  return res.isSuccess ? success({ message: STRINGS.open.note_opened }, res.result.path) : res;
}
...
async function handlePrepend(incomingParams) {
  ...
  async function prependAsRequested(filepath) {
    if (belowHeadline) {
      return await prependNoteBelowHeadline(filepath, belowHeadline, content, ensureNewline);
    }
    return await prependNote(filepath, content, ensureNewline, ignoreFrontMatter);
  }
  const resDNP = getDailyNotePathIfPluginIsAvailable();
  if (resDNP.isSuccess) {
    const filepath = resDNP.result;
...
async function handleList2(incomingParams) {
  const paths = getFileMap().filter((t) => t instanceof import_obsidian5.TFolder).map((t) => t.path.endsWith("/") ? t.path : `${t.path}/`).sort();
...
async function handleInfo(incomingParams) {
  ...
  const { isAndroidApp, isDesktopApp, isIosApp, isMacOS } = import_obsidian6.Platform;
  let platform = "";
  ...
  return success({
    ...PLUGIN_INFO,
    apiVersion: import_obsidian6.apiVersion,
...
var routePath5 = {
  "/note": [
    ...
    { path: "/create", schema: createParams3, handler: handleCreate3 },
...
async function handlePrepend2(incomingParams) {
  ...
  async function prependAsRequested() {
    if (belowHeadline) {
      return await prependNoteBelowHeadline(file, belowHeadline, content, ensureNewline);
    }
...
async function handleOpen2(incomingParams) {
  const params = incomingParams;
  window.open("obsidian://omnisearch?vault=" + encodeURIComponent(window.app.vault.getName()) + "&query=" + encodeURIComponent(params.query.trim()));
...
var routePath9 = {
  "/vault": [
    helloRoute(),
    { path: "/open", schema: defaultParams4, handler: handleOpen4 },
    { path: "/close", schema: defaultParams4, handler: handleClose },
...
var routePath10 = {
  "/tags": [
    helloRoute(),
    { path: "/list", schema: listParams4, handler: handleList4 }
  ]
};
...
var ActionsURI = class extends import_obsidian9.Plugin {
  ...
  registerRoutes(routeTree) {
    ...
    for (const [routePath11, routeSubpaths] of Object.entries(routeTree)) {
      for (const route of routeSubpaths) {
        const { path, schema, handler } = route;
        const fullPath = (0, import_obsidian9.normalizePath)(`${URI_NAMESPACE}/${routePath11}/${path}`).replace(/\/$/, "");
  ...
  sendUrlCallbackIfNeeded(handlerRes, params) {
    if (handlerRes.isSuccess) {
      return params["x-success"] ? sendUrlCallback(params["x-success"], handlerRes, params) : success("No `x-error` callback URL provided");
    }
  ...
  async openFileIfNeeded(handlerResult, params) {
    ...
    if (!processedFilepath) {
      return success("No file to open, handler didn't return a `processedFilepath` property");
    }
