---
ID: "dae8cf3f-1904-4e4e-a512-ddb6e844ecca"
Date Created: "2023-03-22"
Date Modified: "2023-03-22"
Author: Jimmy Briggs <jimmy.briggs@jimbrig.com>
Title: "Windows Command Line Commands Overview.md"
Sources: 
  - [""](""): ""
ImageURL: ""
Tags: []
...
Most files in Windows are associated with a specific program that is assigned to open the file by default. At times, remembering these associations can become confusing. You can remind yourself by entering the command **assoc**Â to display a full list of file name extensions and program associations.
...
The cipher command, however, wipes a directory by writing random data to it. To wipe yourÂ CÂ drive, for example, you'd use the **cipher /w:d**Â command, which will wipe free space on the drive. The command does not overwrite undeleted data, so you will not wipe out files you need by running this command.
...
You can also extend the command in several ways. Typing **/b**Â compares only binary output, **/c**Â disregards the case of text in the comparison, and **/l**Â only compares ASCII text.
...
Still, ipconfig is useful because of its extensions. **ipconfig /release**Â followed by **ipconfig /renew**Â can force your Windows PC into asking for a new IP address, which is useful if your computer claims one isn't available. You can also use **ipconfig /flushdns**Â to refresh your DNS address. These commands are great if the [Windows network troubleshooter] chokes, which does happen on occasion.

## [Netstat]

Entering the command **netstat -an**Â will provide you with a list of currently open ports and related IP addresses. This command will also tell you what state the port is in; listening, established, or closed.
...
Typing **ping**Â followed by an IP address or web domain will send a series of test packets to the specified address. If they arrive and are returned, you know the device is capable of communicating with your PC; if it fails, you know that there's something blocking communication between the device and your computer. This can help you decide if the root of the issue is an improper configuration or a failure of network hardware.
...
Powercfg is a very powerful command for managing and tracking how your computer uses energy. You can use the command **powercfg hibernate on**Â and **powercfg hibernate off**Â to manage hibernation, and you can also use the command **powercfg /a**Â to view the power-saving states currently available on your PC.
...
Windows 8 introduced the shutdown command that, you guessed it,Â [shuts down your computer].

This is, of course, redundant with the already easily accessed shutdown button, but what's not redundant is the **shutdown /r /o**Â command, which restarts your PC and launches the Advanced Start Options menu, which is where you can access Safe Mode and Windows recovery utilities. This is useful if you want to restart your computer for troubleshooting purposes.
...
Use **systeminfo /s**Â followed by the host name of a computer on your local network, to remotely grab the information for that system. This may require additional syntax elements for the domain, user name, and password, like this:
...
You can use the **tasklist**Â command to provide a current list of all tasks running on your PC. Though somewhat redundant with Task Manager, the command may sometimes find tasks hidden from view in that utility.

There's also a wide range of modifiers. **Tasklist -svc**Â shows services related to each task, use **tasklist -v**Â to obtain more detail on each task, and **tasklist -m**Â will locateÂ DLL files associated with active tasks. These commands are useful for advanced troubleshooting.

Our reader Eric noted that you can "get the name of the executable associated with the particular process ID you're interested in." The command for that operation isÂ **tasklist | find \[process id].**
...
Windows automatically marks your driveÂ for a diagnostic chkdsk scan when symptoms indicate thatÂ a local driveÂ has bad sectors, lost clusters, or other logical or physical errors.
...
**Schtasks** is your command prompt access to the Task Scheduler, one of many underrated Windows administrative tools. While you can use the GUI to manage your scheduled tasks, the command prompt lets you copy&paste complex commands to set up multiple similar tasks without having to click through various options. Ultimately, it's much easier to use, once you've committedÂ keyÂ parameters toÂ memory.

For example, you could schedule your computer to reboot at 11pmÂ every Friday:

```cmd
schtasks /create /scÂ weekly /d FRIÂ /tn "auto reboot computer weekly" /st 23:00 /tr "shutdown -r -f -t 10"
```

To complement your weekly reboot, you couldÂ schedule tasks to launch specific programs on startup:

```cmd
schtasks /create /sc onstart /tn "launchÂ Chrome on startup" /tr "C:\Program Files (x86)\Google\Chrome\Application\Chrome.exe"
```

ToÂ duplicate the above command for different programs, just copy, paste, and modify it asÂ needed.
